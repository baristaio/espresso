...
const postMessages = (logger, amqp, queueName, messages) => {
  return new Promise(((resolve, reject) => {
    amqp.createChannel((err, ch) => {
      ...
      messages.forEach(m => ch.sendToQueue(queueName, Buffer.from(m)));
      logger.debug(`Messages posted to queue ${queueName}`);
      ch.close();
      resolve();
    });
  }));
