...
function getConnections(logger, connectionsDescriptor, { app }) {
  return new Promise((resolve) => {
    ...
    connectionsDescriptor.forEach(descriptor => {
      promises.push(mountService(logger, descriptor, app).then(connection => ({
...
const getService = (serviceDescriptor, logLevel = 'info') => {
  ...
  const start = async() => {
    ...
    return new Promise(async(resolve, reject) => {
      try {
        const connections = await getConnections(logger, serviceDescriptor.connections, { app });
